    ...
    try:
        ...
        for user in users:
            try:
                ...
                logger.info(f"✅ Notification sent to user {tg_id}, type: {notification_type}, time_left: {days_left}d {hours_left}h")

                # Отмечаем что уведомление отправлено и устанавливаем следующее
                await db.mark_notification_sent(tg_id)
                logger.info(f"✅ Next notification scheduled for user {tg_id}")
