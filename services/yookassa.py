    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            if not uuid:
                ...
                await db.update_payment_status_by_invoice(payment_id, 'failed')
            ...
            logging.info(f"Yookassa payment processed successfully: user={tg_id}, tariff={tariff_code}, days={days}")
            return True

    except asyncio.TimeoutError:
        # ⚠️ ОТКАТЫВАЕМ ПЛАТЁЖ при таймауте
        await db.update_payment_status_by_invoice(payment_id, 'failed')
        logging.error(f"Timeout while processing Yookassa payment for {tg_id}, payment marked as failed")
        try:
            await bot.send_message(
                tg_id,
        ...
        await db.update_payment_status_by_invoice(payment_id, 'failed')
        logging.error(f"Process Yookassa payment exception: {e}, payment marked as failed")
        try:
            await bot.send_message(
                tg_id,
