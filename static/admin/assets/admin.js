...
async function loadUsers() {
  try {
    ...
    $("usersTable").innerHTML = d.items.length ? `<table><thead><tr><th>Пользователь</th><th>Создан</th><th>Подписки</th><th>Ближайший срок</th><th>Выручка</th><th></th></tr></thead><tbody>${d.items.map((u) => `<tr><td><b>${esc(u.username || "Без username")}</b><br><span class="mono muted">${u.tg_id}</span></td><td>${date(u.created_at)}</td><td><span class="badge ${Number(u.active_subscriptions) ? "" : "off"}">${u.active_subscriptions} активных / ${u.subscriptions_count}</span></td><td>${date(u.latest_expiry)}</td><td>${money(u.revenue)}</td><td><button class="button small blue" data-user="${u.tg_id}">Открыть</button></td></tr>`).join("")}</tbody></table>` : `<div class="empty">Пользователи не найдены</div>`;
...
async function openUser(tgId) {
  try {
    const d = await api(`/users/${tgId}`); const u = d.user; $("dialogTitle").textContent = u.username ? `@${u.username}` : String(u.tg_id);
    ...
    $("userDetails").innerHTML = `<div class="details-body"><div class="details-grid"><div class="detail-tile"><span>Telegram ID</span><b class="mono">${u.tg_id}</b></div><div class="detail-tile"><span>Регистрация</span><b>${date(u.created_at, true)}</b></div><div class="detail-tile"><span>Источник</span><b>${esc(u.tracking_code || "—")}</b></div></div><div><h3>Подписки</h3><div class="subscription-list">${subs || `<div class="empty">Подписок нет</div>`}</div></div><div><h3>Последние платежи</h3><div class="table-wrap">${payments}</div></div></div>`;
...
$("linkForm").addEventListener("submit", async (e) => { e.preventDefault(); const data = Object.fromEntries(new FormData(e.target)); try { const d = await api("/links", { method: "POST", body: JSON.stringify(data) }); await navigator.clipboard.writeText(d.url).catch(() => {}); toast("Ссылка создана и скопирована"); e.target.reset(); loadLinks(); } catch (error) { toast(error.message, true); } });
async function loadLinks() { try { const d = await api("/links"); $("linksTable").innerHTML = d.items.length ? `<table><thead><tr><th>Ссылка</th><th>Переходы</th><th>Пользователи</th><th>Выручка</th><th>Статус</th><th></th></tr></thead><tbody>${d.items.map((l) => { const url = `https://t.me/${d.bot_username}?start=${l.code}`; return `<tr><td><b>${esc(l.title || l.code)}</b><br><button class="button small mono" data-copy="${esc(url)}">${esc(l.code)}</button></td><td>${l.clicks} / ${l.unique_clicks} уник.</td><td>${l.users_count}</td><td>${money(l.revenue)}</td><td><span class="badge ${l.is_active ? "" : "off"}">${l.is_active ? "Активна" : "Выключена"}</span></td><td><button class="button small" data-link="${esc(l.code)}" data-active="${!l.is_active}">${l.is_active ? "Выключить" : "Включить"}</button></td></tr>`; }).join("")}</tbody></table>` : `<div class="empty">Ссылок пока нет</div>`; } catch (error) { toast(error.message, true); } }
