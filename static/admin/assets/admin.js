...
async function openUser(tgId) {
  try {
    ...
    const subs = d.subscriptions.map((s) => `<article class="subscription-card"><div><h3>${s.plan_kind === "bypass" ? "С антиглушилкой" : "Обычная"} #${s.type_index || s.slot_number}</h3><small>ID ${s.id} · до ${date(s.subscription_until, true)}</small><div><span class="badge ${isActiveDate(s.subscription_until) ? "" : "off"}">${isActiveDate(s.subscription_until) ? "Активна" : "Истекла"}</span></div></div><div class="subscription-controls"><div class="actions"><button type="button" class="button small" data-adjust="${s.id}" data-days="7">+7 дней</button><button type="button" class="button small" data-adjust="${s.id}" data-days="30">+30 дней</button><button type="button" class="button small danger" data-delete-sub="${s.id}">Удалить</button></div><div class="custom-days"><input data-days-input="${s.id}" type="number" min="-3650" max="3650" placeholder="+30 или −15" /><button type="button" class="button small blue" data-custom-adjust="${s.id}">Изменить срок</button></div></div></article>`).join("");
